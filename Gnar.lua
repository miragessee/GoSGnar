...
function QDmg()
    if myHero:GetSpellData(_Q).level == 0 then
        local Dmg1 = (({5, 45, 85, 125, 165})[1] + 1.15 * myHero.totalDamage)
        return Dmg1
    else
        local Dmg1 = (({5, 45, 85, 125, 165})[myHero:GetSpellData(_Q).level] + 1.15 * myHero.totalDamage)
        return Dmg1
    end
end
...
function RDmg()
    if myHero:GetSpellData(_R).level == 0 then
        local Dmg1 = (({200, 300, 400})[1] + 0.2 * myHero.bonusDamage + 0.5 * myHero.ap)
        return Dmg1
    else
        local Dmg1 = (({200, 300, 400})[myHero:GetSpellData(_R).level] + 0.2 * myHero.bonusDamage + 0.5 * myHero.ap)
        return Dmg1
    end
end
...
local HeroIcon = "https://www.mobafire.com/images/champion/icon/gnar.png"
local IgniteIcon = "http://pm1.narvii.com/5792/0ce6cda7883a814a1a1e93efa05184543982a1e4_hq.jpg"
local QIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/7/74/Boomerang_Throw.png"
local WIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/f/f2/Hyper.png"
local EIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/3/3f/Hop.png"
local RIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/3/35/GNAR%21.png"
local IS = {}
local Spells = {
    ["Aatrox"] = {"AatroxE"},
...
function Gnar:LoadMenu()
    ...
    self.GnarMenu = MenuElement({type = MENU, id = "Gnar", name = "Mirage's Gnar", leftIcon = HeroIcon})
    
    self.GnarMenu:MenuElement({id = "Harass", name = "Harass", type = MENU})
    self.GnarMenu.Harass:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    self.GnarMenu.Harass:MenuElement({id = "UseW", name = "Use W", value = true, leftIcon = WIcon})
    ...
    self.GnarMenu.AutoLevel:MenuElement({id = "AutoLevel", name = "Only Q->W->E", value = true})
    
    self.GnarMenu:MenuElement({id = "Escape", name = "Escape", type = MENU})
    self.GnarMenu.Escape:MenuElement({id = "UseE", name = "Use E", value = true})
    ...
    self.GnarMenu:MenuElement({id = "blank", type = SPACE, name = "Script Ver: " .. Version .. " - LoL Ver: " .. LVersion .. ""})
    self.GnarMenu:MenuElement({id = "blank", type = SPACE, name = "by " .. Author .. ""})
end

function Gnar:LoadSpells()
    GnarQ = {delay = 0.25, speed = 2500, radius = 55, range = 1100}
    GnarQReturn = {delay = 0.25, speed = 1700, radius = 75, range = 3000}
    GnarE = {delay = 0.25, speed = 900, radius = 160, range = 475}
    GnarBigQ = {delay = 0.5, speed = 2100, radius = 90, range = 1100}
    GnarBigW = {delay = 0.6, speed = math.huge, radius = 100, range = 550}
    GnarBigE = {delay = 0.25, speed = 800, radius = 375, range = 600}
    GnarR = {delay = 0.25, speed = math.huge, radius = 80, range = 475}
...
function Gnar:Tick()
    ...
    if self.GnarMenu.AutoLevel.AutoLevel:Value() then
...
function Gnar:CollisionX(myHeroPos, dangerousPos, unitPos, radius)
...
function Gnar:CalculateEndPos(startPos, placementPos, unitPos, range)
    if range > 0 then
...
function Gnar:ProcessSpell(units)
    for i = 1, #units do
        local unit = units[i]
    ...
    for key, v in pairs(IS) do
        ...
        if GetDistanceSqr(SpellHit, SpellPosition) <= GetDistanceSqr(dodge, SpellPosition) and GetDistance(SpellHit, v.sPos) - v.radius - myHero.boundingRadius <= GetDistance(v.sPos, v.ePos) then
            if GetDistanceSqr(myHero.pos, SpellHit) < (v.radius + myHero.boundingRadius) ^ 2 then
                if self.GnarMenu.Escape.UseE:Value() then
                    if IsReady(_E) then
...
function Gnar:KillSteal()
    ...
    if self.GnarMenu.KillSteal.UseQ:Value() then
        if IsReady(_Q) then
            for i, enemy in pairs(GetEnemyHeroes()) do
                if ValidTarget(enemy, GnarQ.range) and enemy.health < QDmg() then
    ...
    if self.GnarMenu.KillSteal.UseR:Value() then
        if IsReady(_R) then
            for i, enemy in pairs(GetEnemyHeroes()) do
                if ValidTarget(enemy, GnarR.range) and enemy.health < RDmg() then
...
function Gnar:Harass()
    ...
    if targetQ then
        if not IsImmune(targetQ) then
            if self.GnarMenu.Harass.UseQ:Value() then
                if IsReady(_Q) and self.Collision == false then
                    if ValidTarget(targetQ, GnarQ.range) and GetSpellQName() == "GnarQ" then
                        local hitChance, aimPosition = HPred:GetHitchance(myHero.pos, targetQ, GnarQ.range, GnarQ.delay, GnarQ.speed, GnarQ.radius, false)
...
function Gnar:CastQ(target, EcastPos)
    if LocalGameTimer() - OnWaypoint(target).time > 0.05 and (LocalGameTimer() - OnWaypoint(target).time < 0.125 or LocalGameTimer() - OnWaypoint(target).time > 1.25) then
        if GetDistance(myHero.pos, EcastPos) <= GnarQ.range then
...
function Gnar:Combo()
    ...
    if targetQ then
        if not IsImmune(targetQ) then
            if self.GnarMenu.Combo.UseQ:Value() then
                if IsReady(_Q) and self.Collision == false then
                    if ValidTarget(targetQ, GnarQ.range) and GetSpellQName() == "GnarQ" then
                        local hitChance, aimPosition = HPred:GetHitchance(myHero.pos, targetQ, GnarQ.range, GnarQ.delay, GnarQ.speed, GnarQ.radius, false)
                        if hitChance and hitChance >= 2 then
    ...
    if targetR then
        if not IsImmune(targetR) then
            if self.GnarMenu.Combo.UseR:Value() then
                if IsReady(_R) then
                    if ValidTarget(targetR, GnarR.range) and targetR.health < RDmg() then
